...
{
    public class AssignDeAssignUserPlanProcedureCommandHandler : IRequestHandler<AssignDeAssignUserPlanProcedureCommand, ApiResponse<Unit>>
    {
        private readonly RLContext _context;
        public AssignDeAssignUserPlanProcedureCommandHandler(RLContext context)
        {
            _context = context;
        }
        public async Task<ApiResponse<Unit>> Handle(AssignDeAssignUserPlanProcedureCommand request, CancellationToken cancellationToken)
        {
            try
            {
                //Validate request
                ...
                if (request.AssignmentType == AssignmentType.DeAssign)
                {
                    if (!userExistence)
